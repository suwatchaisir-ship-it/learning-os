...
def generate_line_message_with_ai(articles, dashboard_url):
    ...
    if not model:
        ...
        prompt = "แปลหัวข้อข่าวต่อไปนี้เป็นภาษาไทยให้สั้นกระชับ สละสลวย เพื่อส่งเข้า LINE:\n\n"
        for art in articles:
            prompt += f"- {art['title']}\n"
        prompt += "\nส่งกลับมาเฉพาะหัวข้อข่าวภาษาไทยที่มีอีโมจิ 🔹 นำหน้าแต่ละบรรทัดเรียงลงมาเลย ไม่ต้องมีคำเกริ่นนำใดๆ"
        
        try:
            response = model.generate_content(prompt)
            base_msg += response.text.strip() + "\n"
        except Exception as e:
